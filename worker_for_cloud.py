...
modelpath='/home/som/lab/jmr/BP-Net/saved_models/0BP715.hdf5'##########################################################################################################
...
def bpcalculation(bprecording1,bprecording2,bprecording3,bprecording4,bprecording5):#,bprecording5
    ...
    print('血压', bp[0])
    # write = open('myzhibiao.csv', 'a', newline='', encoding='gb18030')
...
conn = BaseManager(address = ('114.213.234.31',8786), authkey = b'123');
try:
    conn.connect();
except:
    print('连接失败');
