...
def load_data(path): ###归一化
    ...
    for i in range(len(date)):
        ...
        for j in range(len(vonlunteers)):
            ...
            bmirecording = loadbmi(bmipath, vonlunteers[j].split('_')[0])###############################################################################huifu
            bmiarr.append(bmirecording)###############################################################################huifu
    ...
    bmiarr = np.array(bmiarr)###############################################################################huifu
    print("bmiarr",bmiarr.shape)###############################################################################huifu
    # bmiarr = bmiarr[:, np.newaxis]
...
# x_val,y_val,bmi_val, myfft_val=load_data(val_path)
print('验证集数据维度如下!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
print(x_val.shape)
print(y_val.shape)
x_test,x1_test,x2_test,x3_test,y_test, bmi_test=load_data(test_path)
# x_test,y_test,bmi_test, myfft_test=load_data(test_path)
...
if not data_augmentation:
    ...
    model.fit([x_train,x1_train,x2_train,x3_train,bmi_train], y_train, batch_size=batch_size, epochs=epochs,#######################,bmi_train
              validation_data=([x_val,x1_val,x2_val,x3_val,bmi_val], y_val),shuffle=True,callbacks=callbacks) ####################,bmi_val

else:
    print('Using real-time data augmentation.')

    model.fit([x_train, x1_train,x2_train,x3_train,bmi_train], y_train, batch_size=8, epochs=300, verbose=1,
              validation_data=([x_val,x1_val,x2_val,x3_val,bmi_val], y_val))
end = time.time()

scores = model.evaluate([x_val,x1_val,x2_val,x3_val,bmi_val], y_val, verbose=1)##########################,bmi_val
